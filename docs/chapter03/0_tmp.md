...
$$Ber(y|\theta) \coleq \theta^{y} (1 - \theta)^{1-y}  \qquad{(3.1)}$$
...
$$Bin(s|N,\theta) \coleq \begin{pmatrix} N \\ s \end{pmatrix} \theta^s (1 - \theta)^{N-s} \qquad{(3.3)}$$
...
$$p(y|\bm{x}, \bm{\theta}) = \textrm{Ber}(y|\sigma(f(\bm{x};\bm{\theta})))$$
